...
class BaseModel(object):
    ...
    def finalize(self, obj=obj):

        for yaml_model in self.obj:
            fields = {'__str__': lambda self: '#%d - %s' % (self.id, yaml_model['verbose_name'])}
            ...
            model = self.create_model(yaml_model['verbose_name'], fields,
                                      meta_opts=meta_opts, admin_opts=admin_opts,
                                      app_label='hyperdrive', module=self.__module__, )
